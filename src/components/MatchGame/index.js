...
class MatchGame extends Component {
  ...
  matchGameRun = () => {
    const {randomImgId, activeId} = this.state
    const randomImg = imagesList.filter(each => each.id === randomImgId)
    const filterList = imagesList.filter(each => each.category === activeId)
    return (
      <div className="bg-container">
        <div className="match-container">
          <img
            src={randomImg[0].imageUrl}
            alt="match"
            className="random-image"
          />
          <ul className="tabs-container">
            {tabsList.map(each => {
              const activeTab = activeId === each.tabId ? 'active-btn' : ''
              const onChangeTab = () => {
                this.setState({activeId: each.tabId})
              }
              return (
                <li key={each.tabId}>
                  <button
          ...
          <ul className="images-container">
            {filterList.map(each => {
              const onSelectImage = () => {
                console.log(each.id, randomImgId)
                if (each.id === randomImgId) {
                  this.setState(prev => ({
                    score: prev.score + 1,
                    randomImgId:
              ...
              return (
                <li className="image-item" key={each.id}>
                  <button
                    type="button"
                    className="image-btn"
  ...
  render() {
    const {score, timer, gameRun} = this.state
    return (
      <div className="background">
        ...
        {gameRun ? (
          ...
          <GameOver score={score} resetGame={this.resetGame} />
        )}
      </div>
    )
  }
